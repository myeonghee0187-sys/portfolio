/*
 * React Bits — Magic Rings (TS / CSS 변형)
 * 출처: https://reactbits.dev/animations/magic-rings
 *       DavidHDev/react-bits · src/ts-default/Animations/MagicRings/MagicRings.tsx
 *
 * 셰이더와 렌더 루프는 원본 그대로이고, 이 프로젝트에 맞춘 수정은 아래 한 가지뿐이다.
 *  - 마우스 이벤트를 컨테이너가 아니라 window에서 받는다.
 *    배경 레이어에 pointer-events: none을 줘야 Header/CONTACT/Watch의 클릭을
 *    가로채지 않는데, 그러면 컨테이너에 붙인 원본 리스너는 영영 발화하지 않는다.
 *    rect로 안/밖을 판정해 hover(mouseenter/mouseleave) 동작까지 같게 재현한다.
 */
...
export default function MagicRings({
  ...
  useEffect(() => {
    ...
    // 배경 레이어가 pointer-events: none이라 컨테이너 리스너는 발화하지 않는다.
    // window에서 받아 rect로 안/밖을 판정한다. 좌표 계산은 원본과 동일하다.
    const isInside = (e: MouseEvent, rect: DOMRect) =>
      e.clientX >= rect.left && e.clientX <= rect.right && e.clientY >= rect.top && e.clientY <= rect.bottom;

    const onMouseMove = (e: MouseEvent) => {
      const rect = mount.getBoundingClientRect();
      const inside = isInside(e, rect);
      isHoveredRef.current = inside;
      // 밖으로 나가면 원본의 mouseleave와 같이 중심으로 되돌린다.
      mouseRef.current[0] = inside ? (e.clientX - rect.left) / rect.width - 0.5 : 0;
      mouseRef.current[1] = inside ? -((e.clientY - rect.top) / rect.height - 0.5) : 0;
    };
    const onClick = (e: MouseEvent) => {
      if (isInside(e, mount.getBoundingClientRect())) burstRef.current = 1;
    };
